...
CREATE TABLE room_import_hacks (
    ...
    message_id_offset INTEGER NOT NULL
                ...
                for row in conn.execute("SELECT room, old_message_id_max, message_id_offset FROM old_room_import_hacks"):
                    print(f"r: {row['room']}, {row['old_message_id_max']}, {row['message_id_offset']}")
                conn.execute(
                    """
